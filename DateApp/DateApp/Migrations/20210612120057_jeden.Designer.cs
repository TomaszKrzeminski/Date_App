// <auto-generated />
using DateApp.Models;
using Microsoft.EntityFrameworkCore;
using Microsoft.EntityFrameworkCore.Infrastructure;
using Microsoft.EntityFrameworkCore.Metadata;
using Microsoft.EntityFrameworkCore.Migrations;
using Microsoft.EntityFrameworkCore.Storage;
using Microsoft.EntityFrameworkCore.Storage.Internal;
using System;

namespace DateApp.Migrations
{
    [DbContext(typeof(AppIdentityDbContext))]
    [Migration("20210612120057_jeden")]
    partial class jeden
    {
        protected override void BuildTargetModel(ModelBuilder modelBuilder)
        {
#pragma warning disable 612, 618
            modelBuilder
                .HasAnnotation("ProductVersion", "2.0.3-rtm-10026")
                .HasAnnotation("SqlServer:ValueGenerationStrategy", SqlServerValueGenerationStrategy.IdentityColumn);

            modelBuilder.Entity("DateApp.Models.AppUser", b =>
                {
                    b.Property<string>("Id")
                        .ValueGeneratedOnAdd();

                    b.Property<int>("AccessFailedCount");

                    b.Property<int>("Age");

                    b.Property<string>("City");

                    b.Property<string>("ConcurrencyStamp")
                        .IsConcurrencyToken();

                    b.Property<int>("CoordinatesId");

                    b.Property<DateTime>("Dateofbirth");

                    b.Property<string>("Email")
                        .HasMaxLength(256);

                    b.Property<bool>("EmailConfirmed");

                    b.Property<string>("FirstName");

                    b.Property<bool>("LockoutEnabled");

                    b.Property<DateTimeOffset?>("LockoutEnd");

                    b.Property<string>("NormalizedEmail")
                        .HasMaxLength(256);

                    b.Property<string>("NormalizedUserName")
                        .HasMaxLength(256);

                    b.Property<int>("NotificationCheckId");

                    b.Property<string>("PasswordHash");

                    b.Property<string>("PhoneNumber");

                    b.Property<bool>("PhoneNumberConfirmed");

                    b.Property<bool>("PremiumVersion");

                    b.Property<int>("SearchDetailsId");

                    b.Property<string>("SecurityStamp");

                    b.Property<string>("Sex");

                    b.Property<string>("Surname");

                    b.Property<bool>("TestVersion");

                    b.Property<bool>("TestVersionUsed");

                    b.Property<DateTime>("TimePremium");

                    b.Property<DateTime>("TimeTest");

                    b.Property<bool>("TwoFactorEnabled");

                    b.Property<string>("UserName")
                        .HasMaxLength(256);

                    b.HasKey("Id");

                    b.HasIndex("NormalizedEmail")
                        .HasName("EmailIndex");

                    b.HasIndex("NormalizedUserName")
                        .IsUnique()
                        .HasName("UserNameIndex")
                        .HasFilter("[NormalizedUserName] IS NOT NULL");

                    b.ToTable("AspNetUsers");
                });

            modelBuilder.Entity("DateApp.Models.Coordinates", b =>
                {
                    b.Property<int>("CoordinatesId")
                        .ValueGeneratedOnAdd();

                    b.Property<string>("AppUserId");

                    b.Property<double>("Latitude");

                    b.Property<double>("Longitude");

                    b.Property<string>("UserId");

                    b.HasKey("CoordinatesId");

                    b.HasIndex("AppUserId")
                        .IsUnique()
                        .HasFilter("[AppUserId] IS NOT NULL");

                    b.ToTable("Coordinates");
                });

            modelBuilder.Entity("DateApp.Models.Event", b =>
                {
                    b.Property<int>("EventId")
                        .ValueGeneratedOnAdd();

                    b.Property<string>("City")
                        .IsRequired();

                    b.Property<DateTime>("Date");

                    b.Property<string>("Description")
                        .IsRequired();

                    b.Property<string>("EventName")
                        .IsRequired();

                    b.Property<string>("FilePath");

                    b.Property<string>("Latitude");

                    b.Property<string>("Longitude");

                    b.Property<string>("OrganizerEmail");

                    b.Property<string>("PhotoPath1");

                    b.Property<string>("PhotoPath2");

                    b.Property<string>("PhotoPath3");

                    b.Property<string>("ZipCode")
                        .IsRequired();

                    b.HasKey("EventId");

                    b.ToTable("Events");
                });

            modelBuilder.Entity("DateApp.Models.EventUser", b =>
                {
                    b.Property<string>("AppUserId");

                    b.Property<int>("EventId");

                    b.HasKey("AppUserId", "EventId");

                    b.HasIndex("EventId");

                    b.ToTable("EventUser");
                });

            modelBuilder.Entity("DateApp.Models.LoginHistory", b =>
                {
                    b.Property<int>("LoginHistoryId")
                        .ValueGeneratedOnAdd();

                    b.Property<string>("ApplicationUserID");

                    b.Property<DateTime>("LoggedIn");

                    b.Property<DateTime>("LoggedOut");

                    b.Property<string>("UserId");

                    b.HasKey("LoginHistoryId");

                    b.HasIndex("UserId");

                    b.ToTable("LoginHistory");
                });

            modelBuilder.Entity("DateApp.Models.Match", b =>
                {
                    b.Property<int>("MatchId")
                        .ValueGeneratedOnAdd();

                    b.Property<string>("AcceptFirst");

                    b.Property<string>("AcceptSecond");

                    b.Property<bool>("DontShow");

                    b.Property<string>("FirstUserId");

                    b.Property<string>("MainPhotoUser1");

                    b.Property<string>("MainPhotoUser2");

                    b.Property<bool>("NewForFirstUser");

                    b.Property<bool>("NewForSecondUser");

                    b.Property<string>("Pair");

                    b.Property<bool>("Reject");

                    b.Property<string>("RejectFirst");

                    b.Property<string>("RejectSecond");

                    b.Property<string>("SecondUserId");

                    b.Property<bool>("ShowMatchScreenU1");

                    b.Property<bool>("ShowMatchScreenU2");

                    b.Property<string>("SuperLikeFirst");

                    b.Property<string>("SuperLikeSecond");

                    b.Property<DateTime>("Time");

                    b.HasKey("MatchId");

                    b.ToTable("Matches");
                });

            modelBuilder.Entity("DateApp.Models.MatchUser", b =>
                {
                    b.Property<string>("AppUserId");

                    b.Property<int>("MatchId");

                    b.HasKey("AppUserId", "MatchId");

                    b.HasIndex("MatchId");

                    b.ToTable("MatchUsers");
                });

            modelBuilder.Entity("DateApp.Models.Message", b =>
                {
                    b.Property<int>("MessageId")
                        .ValueGeneratedOnAdd();

                    b.Property<bool>("Checked");

                    b.Property<string>("MessageText");

                    b.Property<string>("ReceiverId");

                    b.Property<string>("SenderId");

                    b.Property<DateTime>("Time");

                    b.HasKey("MessageId");

                    b.ToTable("Messages");
                });

            modelBuilder.Entity("DateApp.Models.MessageUser", b =>
                {
                    b.Property<string>("AppUserId");

                    b.Property<int>("MessageId");

                    b.HasKey("AppUserId", "MessageId");

                    b.HasIndex("MessageId");

                    b.ToTable("MessageUser");
                });

            modelBuilder.Entity("DateApp.Models.NotificationCheck", b =>
                {
                    b.Property<int>("NotificationCheckId")
                        .ValueGeneratedOnAdd();

                    b.Property<string>("AppUserId");

                    b.Property<bool>("Check");

                    b.Property<DateTime>("LastCheck");

                    b.HasKey("NotificationCheckId");

                    b.HasIndex("AppUserId")
                        .IsUnique()
                        .HasFilter("[AppUserId] IS NOT NULL");

                    b.ToTable("NotificationCheck");
                });

            modelBuilder.Entity("DateApp.Models.ReportUser", b =>
                {
                    b.Property<int>("ReportUserId")
                        .ValueGeneratedOnAdd();

                    b.Property<string>("ApplicationUserID");

                    b.Property<string>("ComplainUserId");

                    b.Property<string>("Reason");

                    b.Property<string>("UserId");

                    b.HasKey("ReportUserId");

                    b.HasIndex("UserId");

                    b.ToTable("ReportUsers");
                });

            modelBuilder.Entity("DateApp.Models.SearchDetails", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd();

                    b.Property<string>("AppUserId");

                    b.Property<string>("CityOfResidence");

                    b.Property<string>("CompanyName");

                    b.Property<string>("Description");

                    b.Property<bool>("HideProfile");

                    b.Property<string>("JobPosition");

                    b.Property<DateTime>("LikeDate");

                    b.Property<int>("Likes");

                    b.Property<string>("Localization");

                    b.Property<string>("MainPhotoPath");

                    b.Property<string>("PhotoPath1");

                    b.Property<string>("PhotoPath2");

                    b.Property<string>("PhotoPath3");

                    b.Property<string>("School");

                    b.Property<int>("SearchAge");

                    b.Property<int>("SearchDistance");

                    b.Property<string>("SearchSex");

                    b.Property<DateTime>("SuperLikeDate");

                    b.Property<int>("SuperLikes");

                    b.Property<string>("UserId");

                    b.HasKey("Id");

                    b.HasIndex("AppUserId")
                        .IsUnique()
                        .HasFilter("[AppUserId] IS NOT NULL");

                    b.ToTable("SearchDetails");
                });

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.IdentityRole", b =>
                {
                    b.Property<string>("Id")
                        .ValueGeneratedOnAdd();

                    b.Property<string>("ConcurrencyStamp")
                        .IsConcurrencyToken();

                    b.Property<string>("Name")
                        .HasMaxLength(256);

                    b.Property<string>("NormalizedName")
                        .HasMaxLength(256);

                    b.HasKey("Id");

                    b.HasIndex("NormalizedName")
                        .IsUnique()
                        .HasName("RoleNameIndex")
                        .HasFilter("[NormalizedName] IS NOT NULL");

                    b.ToTable("AspNetRoles");
                });

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.IdentityRoleClaim<string>", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd();

                    b.Property<string>("ClaimType");

                    b.Property<string>("ClaimValue");

                    b.Property<string>("RoleId")
                        .IsRequired();

                    b.HasKey("Id");

                    b.HasIndex("RoleId");

                    b.ToTable("AspNetRoleClaims");
                });

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.IdentityUserClaim<string>", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd();

                    b.Property<string>("ClaimType");

                    b.Property<string>("ClaimValue");

                    b.Property<string>("UserId")
                        .IsRequired();

                    b.HasKey("Id");

                    b.HasIndex("UserId");

                    b.ToTable("AspNetUserClaims");
                });

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.IdentityUserLogin<string>", b =>
                {
                    b.Property<string>("LoginProvider");

                    b.Property<string>("ProviderKey");

                    b.Property<string>("ProviderDisplayName");

                    b.Property<string>("UserId")
                        .IsRequired();

                    b.HasKey("LoginProvider", "ProviderKey");

                    b.HasIndex("UserId");

                    b.ToTable("AspNetUserLogins");
                });

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.IdentityUserRole<string>", b =>
                {
                    b.Property<string>("UserId");

                    b.Property<string>("RoleId");

                    b.HasKey("UserId", "RoleId");

                    b.HasIndex("RoleId");

                    b.ToTable("AspNetUserRoles");
                });

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.IdentityUserToken<string>", b =>
                {
                    b.Property<string>("UserId");

                    b.Property<string>("LoginProvider");

                    b.Property<string>("Name");

                    b.Property<string>("Value");

                    b.HasKey("UserId", "LoginProvider", "Name");

                    b.ToTable("AspNetUserTokens");
                });

            modelBuilder.Entity("DateApp.Models.Coordinates", b =>
                {
                    b.HasOne("DateApp.Models.AppUser", "User")
                        .WithOne("coordinates")
                        .HasForeignKey("DateApp.Models.Coordinates", "AppUserId");
                });

            modelBuilder.Entity("DateApp.Models.EventUser", b =>
                {
                    b.HasOne("DateApp.Models.AppUser", "AppUser")
                        .WithMany("EventUser")
                        .HasForeignKey("AppUserId")
                        .OnDelete(DeleteBehavior.Cascade);

                    b.HasOne("DateApp.Models.Event", "Event")
                        .WithMany("EventUser")
                        .HasForeignKey("EventId")
                        .OnDelete(DeleteBehavior.Cascade);
                });

            modelBuilder.Entity("DateApp.Models.LoginHistory", b =>
                {
                    b.HasOne("DateApp.Models.AppUser", "User")
                        .WithMany("LoginHistory")
                        .HasForeignKey("UserId");
                });

            modelBuilder.Entity("DateApp.Models.MatchUser", b =>
                {
                    b.HasOne("DateApp.Models.AppUser", "AppUser")
                        .WithMany("MatchUser")
                        .HasForeignKey("AppUserId")
                        .OnDelete(DeleteBehavior.Cascade);

                    b.HasOne("DateApp.Models.Match", "Match")
                        .WithMany("MatchUser")
                        .HasForeignKey("MatchId")
                        .OnDelete(DeleteBehavior.Cascade);
                });

            modelBuilder.Entity("DateApp.Models.MessageUser", b =>
                {
                    b.HasOne("DateApp.Models.AppUser", "AppUser")
                        .WithMany("MessageUser")
                        .HasForeignKey("AppUserId")
                        .OnDelete(DeleteBehavior.Cascade);

                    b.HasOne("DateApp.Models.Message", "Message")
                        .WithMany("MessageUser")
                        .HasForeignKey("MessageId")
                        .OnDelete(DeleteBehavior.Cascade);
                });

            modelBuilder.Entity("DateApp.Models.NotificationCheck", b =>
                {
                    b.HasOne("DateApp.Models.AppUser", "User")
                        .WithOne("Notification")
                        .HasForeignKey("DateApp.Models.NotificationCheck", "AppUserId");
                });

            modelBuilder.Entity("DateApp.Models.ReportUser", b =>
                {
                    b.HasOne("DateApp.Models.AppUser", "User")
                        .WithMany("ReportUsers")
                        .HasForeignKey("UserId");
                });

            modelBuilder.Entity("DateApp.Models.SearchDetails", b =>
                {
                    b.HasOne("DateApp.Models.AppUser", "User")
                        .WithOne("Details")
                        .HasForeignKey("DateApp.Models.SearchDetails", "AppUserId");
                });

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.IdentityRoleClaim<string>", b =>
                {
                    b.HasOne("Microsoft.AspNetCore.Identity.IdentityRole")
                        .WithMany()
                        .HasForeignKey("RoleId")
                        .OnDelete(DeleteBehavior.Cascade);
                });

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.IdentityUserClaim<string>", b =>
                {
                    b.HasOne("DateApp.Models.AppUser")
                        .WithMany()
                        .HasForeignKey("UserId")
                        .OnDelete(DeleteBehavior.Cascade);
                });

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.IdentityUserLogin<string>", b =>
                {
                    b.HasOne("DateApp.Models.AppUser")
                        .WithMany()
                        .HasForeignKey("UserId")
                        .OnDelete(DeleteBehavior.Cascade);
                });

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.IdentityUserRole<string>", b =>
                {
                    b.HasOne("Microsoft.AspNetCore.Identity.IdentityRole")
                        .WithMany()
                        .HasForeignKey("RoleId")
                        .OnDelete(DeleteBehavior.Cascade);

                    b.HasOne("DateApp.Models.AppUser")
                        .WithMany()
                        .HasForeignKey("UserId")
                        .OnDelete(DeleteBehavior.Cascade);
                });

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.IdentityUserToken<string>", b =>
                {
                    b.HasOne("DateApp.Models.AppUser")
                        .WithMany()
                        .HasForeignKey("UserId")
                        .OnDelete(DeleteBehavior.Cascade);
                });
#pragma warning restore 612, 618
        }
    }
}
